export default function Stats({ devices }) {
  ...
  const stats = [
    { label: "FDA-Cleared AI Devices", value: total.toLocaleString() },
    { label: "No Published Evidence", value: `${noPublished} (${((noPublished / total) * 100).toFixed(0)}%)`, alert: true },
    { label: "No Evidence (Any Source)", value: `${noAnyEvidence} (${((noAnyEvidence / total) * 100).toFixed(0)}%)` },
    { label: "Adverse Events", value: hasEvents },
    { label: "Recalls", value: hasRecalls },
  ];

  if (hasRegulatory > 0) {
    stats.push({ label: "Has FDA Clinical Data", value: hasRegulatory });
  }

  if (mature.length > 0) {
    stats.push({
      label: "2+ Yrs Old, Still No Evidence",
      value: `${matureNoPublished}/${mature.length} (${((matureNoPublished / mature.length) * 100).toFixed(0)}%)`,
      alert: matureNoPublished / mature.length > 0.25,
    });
  }
